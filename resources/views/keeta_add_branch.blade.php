...
<script>
    ...
    const brandConfig = {
        1: { conceptID: 1, menuTemplateID: 1 },
        1004: { conceptID: 9, menuTemplateID: 7 },
        2: { conceptID: 3, menuTemplateID: 3 },
        3: { conceptID: 2, menuTemplateID: 2 }
    }
    $(document).ready(function() {
        let selectedBrandId = 1; // Default brand ID
        let selectedconceptID = null;
        let selectedmenuTemplateID = null;

        $('.sync-brand-button').on('click', function() {
            selectedBrandId = $(this).closest('.logo-wrapper').data('brand-id');

            const config = brandConfig[selectedBrandId];
            if (!config) {
                console.error('Missing config for brand:', selectedBrandId);
                return;
            }

            selectedconceptID = config.conceptID;
            selectedmenuTemplateID = config.menuTemplateID;
        });
